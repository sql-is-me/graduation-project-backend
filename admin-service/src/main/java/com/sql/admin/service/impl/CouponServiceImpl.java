...
public class CouponServiceImpl implements CouponService {
    ...
    public long createCoupon(CouponCreateDTO dto) {
        ...
        // linkToken 不为空时，校验唯一性
        if (dto.getLinkToken() != null && !dto.getLinkToken().isBlank()) {
            LambdaQueryWrapper<Coupon> tokenWrapper = new LambdaQueryWrapper<>();
            tokenWrapper.eq(Coupon::getLinkToken, dto.getLinkToken().trim());
            if (couponMapper.selectCount(tokenWrapper) > 0) {
                throw new ServiceException("活动链接Token已被使用，请更换");
            }
        }
        ...
        coupon.setStatus("0");
        if (dto.getLinkToken() != null && !dto.getLinkToken().isBlank()) {
            coupon.setLinkToken(dto.getLinkToken().trim());
        }
