...
public interface TrainingMethodMapper extends BaseMapper<TrainingMethod> {
        ...
        @Select("SELECT tm.tm_id, tm.title, u.nick_name AS coachName, tm.description, tm.create_time AS createTime " +
        ...
        List<TrainingMethodsInfo> selectByStoreId(@Param("storeId") Long storeId);

        @Update("UPDATE training_methods SET status = #{status}, reject_reason = #{rejectReason}, update_time = now() WHERE tm_id = #{tmId}")
        int updateStatus(@Param("tmId") Long tmId, @Param("status") String status,
