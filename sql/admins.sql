...
create table admins (
  ...
  username          varchar(30)     not null                   comment '管理员账�?,
  password          varchar(100)    default ''                 comment '密码',
  nick_name         varchar(30)     not null                   comment '管理员昵�?,
  email             varchar(50)     default ''                 comment '邮箱',
  phone             varchar(11)     default ''                 comment '手机号码',
  sex               char(1)         default '0'                comment '性别�?�?1�?2未知�?,
  avatar            varchar(255)    default ''                 comment '头像URL地址',
  store_id          bigint(20)      default null               comment '所属店铺ID（管理员类型为STORE时必填）',
  admin_type        char(1)         default '1'                comment '管理员类型（0:TOP 超级管理�?/ 1:STORE 店铺管理员）',
  referrer_id       bigint(20)      default null               comment '推荐人ID（关联本表admin_id�?,
  status            char(1)         default '0'                comment '账号状态（0正常 1停用�?,
  login_ip          varchar(128)    default ''                 comment '最后登录IP',
  login_date        datetime                                   comment '最后登录时�?,
...
('topadmin', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '超级管理�?, 'top@admin.com', '13900000001', '0', '/avatar/topadmin.jpg', null, '0', null, '0', '10.0.0.1', sysdate(), sysdate(), null),
